...
g1 <- tableGrob(summary_df2_2)
g2 <- tableGrob(summary_df2_3[,-1])

# Ajoute les titres au-dessus des tableaux
title1 <- textGrob("Résumé - T>>C tau=2", gp = gpar(fontsize = 12, fontface = "bold"))
title2 <- textGrob("Résumé - T>>C otc de poisson, tau=2", gp = gpar(fontsize = 12, fontface = "bold"))
...
Ici nous avons un endpoint principal en faveur du groupe traité, dont les distributions sont $\mathcal{B}_T(0.65)$ ; $\mathcal{B}_C(0.3)$ alors que les 2 autres endpoint sont en faveur du groupe contrôle, les distributions sont les mêmes que précédemment mais avec $\beta=2$ pour favorisé le groupe C.
...
win_edp1=sum(result$summary_ep$Trt_Endpoint2[,2])
loose_edp1=sum(result$summary_ep$Con_Endpoint2[,2])
tie_edp1=4*2500-sum(win_edp1+loose_edp1)

win_edp2=sum(result$summary_ep$Trt_Endpoint1[,2])
loose_edp2=sum(result$summary_ep$Con_Endpoint1[,2])
tie_edp2=tie_edp1-(loose_edp2+win_edp2)
