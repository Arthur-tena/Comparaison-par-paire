...
affect_value_GPC_WO_WR <- function(vecteur1, vecteur2, L, w = rep(1, L), threshold = 0) {
  ...
  for (i in 1:n1) {
    for (j in 1:n2) {
      ...
      if (type1[1] == "continue") {
        diff <- vecteur1[i, 1] - vecteur2[j, 1]
        if (diff > threshold) {
          paire[p, 1] <- "favorable"
        } else if (diff < -threshold) {
          paire[p, 1] <- "défavorable"
        } else {
          paire[p, 1] <- "neutre"
        }
      } else {  # Cas binaire
        if (vecteur1[i, 1] == 1 && vecteur2[j, 1] == 0) {
          paire[p, 1] <- "favorable"
        } else if (vecteur1[i, 1] == 0 && vecteur2[j, 1] == 1) {
          paire[p, 1] <- "défavorable"
        } else {
          paire[p, 1] <- "neutre"
        }
  ...
  for (l in 1:L) {
    for (p in 1:(n1 * n2)) {
      ...
      } else if (l < L) {  # Si c'est "non-informatif", on regarde la colonne suivante
        if (paire[p, l + 1] == "favorable") {
          N_w <- N_w + 1
        } else if (paire[p, l + 1] == "défavorable") {
          N_l <- N_l + 1
        } else if (paire[p, l + 1] == "neutre") {
          N_t <- N_t + 1
        }
      }
    }
  ...
  # Résultat
  data <- data.frame(WinRatio = WR, Win_Prop = P_w, CI=CI, CI_WR=CI_wr, WinOdds = WO, GPC = Delta)
  
  return(data)
}


affect_value_GPC_WO_WR(T_1_5, C_1_5, 2)
```
