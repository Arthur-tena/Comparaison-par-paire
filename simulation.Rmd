...
```{r}
affect_value_GPC_WO_WR2 = function(treatmentdata, controldata, threshold = 0, n_perm = 1000) {
  n1 = nrow(treatmentdata)
  n2 = nrow(controldata)
  L = ncol(treatmentdata)
  ...
  compute_statistics <- function(treat, control) {
    combined = rbind(treat, control)
    perm_index = sample(nrow(combined))
    treat = combined[perm_index[1:n1], ]
    control = combined[perm_index[(n1+1):(n1+n2)], ]
    ...
    for (i in 1:n1) {
      for (j in 1:n2) {
        for (l in 1:L) {
          if (type1[l] == "tte") {
            t_obs1 = extract_tte(treat, l)[,1]
            censure1 = extract_tte(treat, l)[,2]
            t_obs2 = extract_tte(control, l)[,1]
            censure2 = extract_tte(control, l)[,2]
          ...
          if (type1[l] == "continue") {
            diff = treat[i, l] - control[j, l]
            if (diff > threshold) {
              paire[p, l] = "favorable"
            } else if (diff < -threshold) {
              paire[p, l] = "défavorable"
            } else if (is.na(treat[i, l]) || is.na(control[j, l])) {
              paire[p, l] = "non-informative"
            } else {
              paire[p, l] = "neutre"
            }
          ...
          if (type1[l] == "binaire") {
            if (treat[i, l] == 1 && control[j, l] == 0) {
              paire[p, l] = "favorable"
            } else if (treat[i, l] == 0 && control[j, l] == 1) {
              paire[p, l] = "défavorable"
            } else if (is.na(treat[i, l]) || is.na(control[j, l])) {
              paire[p, l] = "non-informative"
            } else {
    ...
    return(list(Delta = Delta, WR = WR, WO = WO))
  }

  results_obs = compute_statistics(treatmentdata, controldata)
...
perm_WR <- numeric(n_perm)

results <- foreach(b = 1:n_perm, .combine = rbind, 
                     .export = c("compute_statistics", "treatmentdata", "controldata"), .packages = "survival") %dopar% {
  perm_results <- compute_statistics(treatmentdata, controldata)
  c(perm_results$Delta, perm_results$WR)
}

perm_Delta <- results[, 1]
perm_WR <- results[, 2]

# Arrêter le cluster
stopCluster(cl)
  p_val_Delta = mean(abs(perm_Delta) >= abs(results_obs$Delta))
  p_val_WR = mean(abs(perm_WR) >= abs(results_obs$WR))

  IC_Delta = quantile(perm_Delta, probs = c(0.025, 0.975), na.rm=TRUE)
  IC_WR = quantile(perm_WR, probs = c(0.025, 0.975), na.rm=TRUE)

  results_final = data.frame(WinRatio = results_obs$WR, WinOdds = results_obs$WO, GPC = results_obs$Delta)
  CI_final = data.frame(row.names = c("Win Ratio", "GPC"), lower = c(IC_WR[1], IC_Delta[1]), upper = c(IC_WR[2], IC_Delta[2]))

  return(list(results = results_final, CI = CI_final, p_value_WR = p_val_WR, p_value_GPC = p_val_Delta))
}
affect_value_GPC_WO_WR2(T_1_4,C_1_4)
affect_value_GPC_WO_WR2(T_2_4,C_2_4)
affect_value_GPC_WO_WR2(T_3_4,C_3_4)
```
...
resultats_eq <- foreach(i = 1, .combine = c, .packages = "survival") %dopar% {
  affect_value_GPC_WO_WR2(T_3_4, C_3_4, 1)
}
resultats_up <- foreach(i = 1, .combine = c, .packages = "survival") %dopar% {
  affect_value_GPC_WO_WR2(T_1_4, C_1_4, 1)
}
resultats_lw <- foreach(i = 1, .combine = c, .packages = "survival") %dopar% {
  affect_value_GPC_WO_WR2(T_2_4, C_2_4, 1)
}
