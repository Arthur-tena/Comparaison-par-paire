...
C_1_4=cbind(gener_continue(1,1),gener_continue(1,1))
...
```{r}
type_variable=function(vecteur1,vecteur2,L){
  type=NULL
  if(length(vecteur1)!=length(vecteur2)){return("Problème, les tailles des 2 vecteurs sont différents")}
  
  if(length(vecteur1)==L){
    ifelse(sapply(vecteur1, is.factor), type="binaire", type="continue")
  }
    return(type)
}
...
affect_value_GPC_WO_WR=function(vecteur1,vecteur2,L,w=rep(1,L),threshold=0){
  cpt=rep(0,L) # un compteur étant le p_ij(l)
  delta=rep(0,L) # delta(l),n la proportion en faveur du traitement cumulé
  Delta=0 # Delta, la statistique d'intérêt dans la GPC, la proportion en faveur du traitement
  count=rep(0,3*L) # compteur servant à calculer le nombre d'évènement favorable, défavorable et neutre pour les WR  et WO
  crit=c("favorable", "défavorable", "neutre", "non-informatif") # paire
  paire=NULL
  for(l in 1:L){
    while(paire=="non-informatif"){
      l=l+1
    }
  }
