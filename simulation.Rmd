...
affect_crit(T_2_5,C_2_5, threshold = 0.5)
```
...
calcul_stat(affect_crit(T_3_5,C_3_5, threshold = 0.25))
```
...
affect_value_GPC_WO_WR2 = function(treatmentdata, controldata, threshold = 0, p.val = c("one.sided", "two.sided"), n_perm = 1000) {
  ...
  paire_perm_list = vector("list", n_perm)
  ...
  set.seed(s)  # Assure que chaque itération génère un échantillon différent
  
  comp_perm = comp
  # Mélanger les groupes de manière aléatoire
  comp_perm$groupe_perm <- sample(comp$groupe, size = length(comp$groupe), replace = FALSE)

  # Séparer les groupes permutés en T et C
  compT = subset(comp_perm, groupe_perm == "T")
  compC = subset(comp_perm, groupe_perm == "C")
  
  # Vérification de la structure après permutation
  if (nrow(compT) == 0 | nrow(compC) == 0) {
    return(c(NA, NA, NA))  # Si l'un des groupes est vide, on retourne des NA
  }

  # Appel de `affect_crit` pour obtenir les paires pour chaque permutation
  paire_perm = affect_crit(compT, compC, threshold)
  
  # Vérification de la validité de `paire_perm`
  if (is.null(paire_perm) | nrow(paire_perm) == 0) {
    return(c(NA, NA, NA))  # Si aucune paire n'est générée, retourner des NA
  }

  # Calcul des statistiques avec `calcul_stat`
  stat_perm = calcul_stat(paire_perm)
  N_w_perm = stat_perm[1]
  N_l_perm = stat_perm[2]
  
  # Calcul de Delta_perm
  Delta_perm = (N_w_perm - N_l_perm) / (n1 * n2)
  
  return(c(N_w_perm, N_l_perm, Delta_perm))  # Retourne les résultats pour cette itération
}
  ...
  Delta_perm <- Delta_perm[!is.na(Delta_perm)]
  ...
  CI_GPC = c(Delta_obs + CI_GPC[1], Delta_obs + CI_GPC[2])
   ...
   hist(Delta_perm, breaks = 30, main = "Distribution de Δ sous H0 (permutation)", xlab = "Δ permuté", col = "lightblue", border = "black", xlim=c(-1,1))
  ...
  if (p.val == "one.sided") {
    p_value_GPC = mean(Delta_perm >= Delta_obs)
  } else {
    p_value_GPC = mean(abs(Delta_perm) >= abs(Delta_obs))
  }

  WR = ifelse(N_l != 0, round(N_w / N_l, 3), NA) 
  WO = ifelse((N_w + N_l + N_t) != 0, round((N_w + 0.5 * N_t) / (N_l + 0.5 * N_t), 3), NA)

  P_w = N_w / (N_w + N_l)
  theta_hat = (N_w + 0.5 * N_t) / (N_w + N_l + N_t)

  sigma_WR = sqrt((P_w * (1 - P_w)) / (N_w + N_l))
  sigma_WO = sqrt(theta_hat * (1 - theta_hat) / (N_w + N_l + N_t))

  z_WR = (P_w - 0.5) / sigma_WR
  z_WO = (theta_hat - 0.5) / sigma_WO

  CI_WR = c(P_w - 1.96 * sigma_WR, P_w + 1.96 * sigma_WR)
  CI_WO = c(WO - 1.96 * sigma_WO, WO + 1.96 * sigma_WO)


  if (p.val == "one.sided") {
    p_value_WR = pnorm(-abs(z_WR))
    p_value_WO = pnorm(-abs(z_WO))
  } else {
    p_value_WR = 2 * pnorm(-abs(z_WR))
    p_value_WO = 2 * pnorm(-abs(z_WO))
  }
...
t1=Sys.time()
affect_value_GPC_WO_WR2(T_3_5,C_3_5, threshold = 0.25 ,p.val = "two.sided")
t2=Sys.time()
t2-t1
```
