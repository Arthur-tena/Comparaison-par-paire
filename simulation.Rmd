...
affect_value_GPC_WO_WR2 = function(treatmentdata, controldata, threshold = 0, p.val = c("one.sided", "two.sided"), n_perm = 1000) {
  ...
  # P-values
  p_value_GPC = ifelse(p.val == "one.sided", mean(Delta_perm >= Delta_obs), mean(abs(Delta_perm) >= abs(Delta_obs)))
  p_value_WR = ifelse(p.val == "one.sided", mean(WR_perm >= WR_obs), mean(abs(WR_perm) >= abs(WR_obs)))
  p_value_WO = ifelse(p.val == "one.sided", mean(WO_perm >= WO_obs), mean(abs(WO_perm) >= abs(WO_obs)))

  # Signification des p-values
...
t1=Sys.time()
affect_value_GPC_WO_WR2(T_2_5,C_2_5, threshold = 0 ,p.val = "two.sided", n_perm = 1000)
t2=Sys.time()
t2-t1
```
