...
# Définition de l'intervalle de valeurs pour l'axe x
...
# Tracé de la première courbe (N(3,1))
plot(x, y1, type = "l", lwd = 2, col = "blue", 
     main = "Densités des lois normales N(3,1) et N(2,2)",
     xlab = "x", ylab = "Densité")

# Ajout de la deuxième courbe (N(2,2))
...
# calcul des statistiques en regardant dans un premier temps si la valeur est  non-infomative 
 for (p in 1:(n1 * n2)) {
  ...
  print(WO)
  print(WO2)
  P_w=round((N_w)/(N_w+N_l),3) # proportion de win
  ...
  z <- (P_w - 0.5) / sigma_emp
  p_val=prop.test(N_w,N_w+N_l)$p.value
  
  p_val_uni<- round(1 - pnorm(z),4)
...
return(list(results=data1,CI=data2, Nb=data3, z_score=z, p_value_unilatéral=p_val_uni, p_value_bilatéral=p_val_bilat,p.value=p_val))
}
```

```{r}
t1=Sys.time()
resultats_up=affect_value_GPC_WO_WR(T_1_4,C_1_4)
resultats_eq=affect_value_GPC_WO_WR(T_3_4, C_3_4)
resultats_lw=affect_value_GPC_WO_WR(T_2_4, C_2_4)
t2=Sys.time()
t_end=t2-t1
t_end
```
...
results <- foreach(i = 1:n_sim, .combine = rbind, .packages = "survival") %dopar% {
  ...
  treatmentdata <- cbind(gener_continue(4, 2), gener_continue(3, 2))
  controldata <- cbind(gener_continue(3, 2), gener_continue(2, 4))
  ...
  result <- affect_value_GPC_WO_WR(treatmentdata_perm, controldata_perm)
...
p_values_bilat <- unname(results[,4])


Theta_values <- N_w_values / (N_w_values + N_l_values)
...
cat("Intervalle de Confiance 95% :", round(IC_lower, 3), "-", round(IC_upper, 3), "\n")
cat("P-valeur (Permutation) :", round(p_val_empirique, 4), "\n")
sum(p_values_bilat==0)
